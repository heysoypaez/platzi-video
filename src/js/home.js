	...
	(async function load() {
		...
		======================================*/
			...
			function videoItemTemplate(movie,category) {
				...
				return(
					 `<div class="primaryPlaylistCarrousel" data-id="${movie.id}" data-category=${category}>
			...
			async function cacheExist(category, reload) {
			...
			// movies Playlist
				...
				//
			async function recallDataAPI(event) {

					 	event.preventDefault()
						const dataNew = await cacheExist("action", true);
			...
			function createButtonTemplate(category) {
				...
				$button.className = "recall-api-movielist"

					$button.addEventListener("click", recallDataAPI)
		...
		function renderRecallDataMovies(index) {

				const button = createButtonTemplate("action")
				$primaryPlaylist[index].insertBefore(button, $primaryPlaylist[index].childNodes[4])

		}

		for (let i = 0; i < $primaryPlaylist.length; i++) {
			renderRecallDataMovies(i)
		}
...
/*NOTAS PERSONALES MIENTRAS PROGRAMO
====================================

APUNTES BLOG APORTE
===================

Clase de Animaciones
...
===================
	...
	Actual... Enfocate en el presente Daniel

		- R [] traer nuevos datos con un boton borrando los almacenados en localStorage

			- [HECHO] Crear el boton
			...
			*/ 
			...
			- [] Colocar el boton dentro de la posición que lo necesito

				- [] ¿En qué posicion lo necesito?

			- [HECHO] Crear o aplicar funcion para llamar de nuevo los datos de la API

			- [HECHO?] Hacerlo sin recargar pagina
	...
	Futuros...
		...
		- [] traer nuevos datos luego de un tiempo con set time out que borre el localStorage
		- [] traer datos con session storage
		- [] pasar de funciones a objetos


	Pasados....

		- [HECHO] desafio para colocar en la playlist de amigos personas al azar usando ramdor user me
		- [HECHO] Y my playlist colocar peliculas


	________________________________________________________________________________ 
	Recuerda los principios Daniel

		1. Un programador divide problemas grandes en problemas chiquitos
		2. Un programador se enfoca en resolver un problema a la vez
